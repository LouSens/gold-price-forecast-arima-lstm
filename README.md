...
- a **full project report in Indonesian** (`reports/laporan.md`): domain, business understanding, data understanding, preparation, modeling and evaluation;
...
│
├── notebooks/
│   ├── gold_price_forecasting.py     Notebook source, percent format (runs as a plain script)
│   └── gold_price_forecasting.ipynb  Generated, executed notebook
│
...
│
├── scripts/
│   ├── download_data.py              Optional CLI download of Yahoo Finance GC=F
│   ├── build_notebook.py             .py → .ipynb (optionally executes it)
│   └── package_report.py             Validates and zips report, notebook and figures
│
├── tests/                            pytest suite (loader, leakage, metrics, models)
│
├── reports/
│   ├── laporan.md                    Project report (Indonesian)
...
│   └── reproducibility.md            Fresh-environment run guide and checks
│
...
No manual step is required: the notebook downloads Yahoo Finance `GC=F` into `data/raw/xauusd_daily.csv` on first run
and creates every output directory. To download from the command line instead (see [data/README.md](data/README.md)):
...
| `results/test_predictions.csv` | Origin date, target date (next trading day), actual and predicted close |
| `results/run_summary.json` | Origin and target periods per split, selected hyperparameters (incl. XGBoost boosting rounds), best model |
...
Test set: **284 one-step-ahead forecasts**. Forecast origins run from **2025-03-13 to 2026-04-28**; the predicted
(target) closes are the next trading days, **2025-03-14 to 2026-04-29**. Origin prices range USD 2,973.60–5,318.40,
entirely above the training maximum of USD 2,093.10. Source: `results/metrics.csv` from the executed notebook (RTX 4050 Laptop GPU).
...
|---|---|---|---|---|---|---|---|---|
| **LSTM (W=30, H=64, L=2), selected** | **24.58** | 75.65 | **49.60** | **1.207** | 56.38 | −0.20 % | 0.338 | 17.87 |
| XGBoost (depth 5, lr 0.05, 31 boosting rounds) | 24.67 | **75.55** | 49.76 | 1.209 | **57.45** | −0.35 % | 0.781 | 1.24 |
| ARIMA(0,1,0) | 24.75 | 75.81 | 49.82 | 1.212 | n/a | 0.00 % | 0.979 | 4.40 |
...
The notebook runs end to end in a fresh environment with no files other than itself: it creates `data/raw`,
`data/processed`, `reports/figures`, `results` and `models`, and downloads the data if the CSV is missing. Seeds are
fixed, so accuracy metrics are identical across runs (runtimes vary slightly).
...
`package_report.py` refuses to build `dist/gold_forecast_report.zip` if the notebook is unexecuted, figures are missing or
placeholders remain. Details: [docs/reproducibility.md](docs/reproducibility.md).
